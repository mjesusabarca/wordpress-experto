...
Estamos hablando de los ***[Must-Use Plugins](https://codex.wordpress.org/Must_Use_Plugins)***, también conocidos como `mu-plugins`. Al igual que los plugins comunes y corrientes, se trata de archivos PHP que van a estar incluidos en la carpeta `wp-content/mu-plugins`. Esta carpeta no viene pre-creada en las instalaciones de WordPress, sino que tenemos que crearla a mano. Una vez que la tenemos, podemos empezar a incluir archivos PHP dentro de ella.
